...
│   │   ├── dao/supabase/       # BaseClientDao + BaseServerDao (abstract data-access base classes)
...
│   │   └── localisation/en/    # Re-exports the merged English dictionary (see Localisation)
...
- **New data access** → `src/modules/<module>/dao/`, extending a `BaseClientDao`/`BaseServerDao` (see DAO layer)
...
barrels are intentionally empty right now (`src/modules/common/index.ts`, `src/core/dao/supabase/index.ts`);
import those targets by full path.

### DAO layer (data access)

Database reads/writes go through DAOs, not raw Supabase calls in services. Two abstract bases live in
`src/core/dao/supabase/` (import by full path — the barrel is empty):

- **`BaseClientDao<Entity>`** (`'use client'`) — browser-side `insert` / `update` / `delete` / `findById`,
  using the browser Supabase client.
- **`BaseServerDao<Entity>`** — server-side `findAll(IGetPaginatedData)` / `findById`, using the server
  Supabase client. `findAll` applies `sortBy` + `sortOrder` ordering.

Both define `protected abstract readonly table` and a shared `unwrap()` that throws on PostgREST errors.
A concrete DAO is a one-liner that pins the table and is exported as a singleton:
...
Module `dao/index.ts` barrels them by name (`productClientDao`, `productServerDao`, `userDao`). Services
depend on DAOs; DAOs depend on the Supabase clients.
...
- **Client services** run only in `'use client'` components (`auth.service.ts`, `product.service.ts`,
  `product-image.service.ts`). `product.service` delegates writes to `productClientDao` and owns the
...
There is no i18n runtime yet — localisation is a **static English dictionary built from per-module files**:
...
- `src/modules/index.ts` spread-merges every module's `en` (plus the `ImageUpload` component's) into one
  dictionary at `rootModule.locales.en`.
- `@localisation` (`src/core/localisation/en`) re-exports that merged `en`.
- **In practice components import their own module's dictionary directly** —
  `import en from '@modules/product/locales/en'`, then `const t = en.someNamespace`. App-level strings live
  in `app/locales/en.ts` and are imported directly by `app/` files (they are **not** in the merged dictionary).
...
- **dao/** — `product.dao.ts` (`productClientDao`, client writes) + `product.server.dao.ts`
  (`productServerDao`, server reads), barreled in `dao/index.ts`.
- **services/** — `product.service.ts` (client: create/update/delete via DAO, Realtime subscription, image
  cleanup), `get-product.service.ts` (server: `fetchProducts` via `productServerDao.findAll`),
...
- Server-only services: suffix `*.server.service.ts`. DAOs: `*.dao.ts` (client) / `*.server.dao.ts` (server).
...
1. **Product update writes a camelCase column** — `IUpdateProduct.imageUrl` is passed straight through
   `productService.updateProduct` → `productClientDao.update(data, id)`, which uses the object keys as
...
4. **Deleting a product orphans its image** — `productService.deleteProduct` now only calls
   `productClientDao.delete(id)`; the old storage-folder cleanup was dropped. Removed products leave their
   `product-image/<id>/...` files in the bucket. Re-add cleanup (via `productImageService`) on delete.

5. **`BaseServerDao.findAll` ignores pagination** — it accepts `IGetPaginatedData` (`page`, `pageSize`)
...
11. **Empty barrels / dead alias** — `src/modules/common/index.ts` and `src/core/dao/supabase/index.ts` are
    empty (import their targets by full path); the `@utils/*` alias resolves to a deleted directory.
...
- Password reset flow.
- Real i18n (the localisation system is English-only, statically merged; no locale switching).
- Pagination in `BaseServerDao.findAll` (params accepted, `.range()` not applied).
